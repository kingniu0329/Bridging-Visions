...
_MODELS = {
    "Model-v1.0": "https://huggingface.co/THUDM/ImageReward/blob/main/ImageReward.pt",
}


def available_models() -> List[str]:
    """Returns the names of available Model models"""
...
def ImageReward_download(url: str, root: str):
    ...
    hf_hub_download(repo_id="THUDM/Model", filename=filename, local_dir=root)
    return download_target


def load(name: str = "Model-v1.0", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None, med_config: str = None):
    """Load a Model model

    Parameters
    ----------
    name : str
        A model name listed by `Model.available_models()`, or the path to a model checkpoint containing the state_dict

    device : Union[str, torch.device]
        The device to put the loaded model

    download_root: str
        path to download the model files; by default, it uses "~/.cache/Model"
    ...
    """
    if name in _MODELS:
        model_path = ImageReward_download(_MODELS[name], download_root or os.path.expanduser("~/.cache/Model"))
    ...
    if med_config is None:
        med_config = ImageReward_download("https://huggingface.co/THUDM/ImageReward/blob/main/med_config.json", download_root or os.path.expanduser("~/.cache/Model"))
...
def available_scores() -> List[str]:
    """Returns the names of available Model scores"""
...
def load_score(name: str = "CLIP", device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None):
    ...
    ----------
    name : str
        A model name listed by `Model.available_models()`
    ...
    model : torch.nn.Module
        The Model model
    """
    model_download_root = download_root or os.path.expanduser("~/.cache/Model")
